  ...
  return (
    <span
      className={`px-2 py-1 text-[9px] font-bold uppercase tracking-[0.18em] ${
        isHot ? "bg-hot text-black" : "border border-black/20 bg-white/90 text-black/70"
      }`}
    >
      {label}
    </span>
...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link
      ...
      className="group relative block border border-black bg-white transition-all duration-300 ease-out hover:-translate-y-1 hover:border-hot hover:shadow-[0_18px_40px_-24px_rgba(255,122,0,0.9)]"
    >
      {/* image area */}
      <div className="relative aspect-square overflow-hidden bg-white">
        ...
        {soldOut && (
          <div className="absolute inset-0 z-10 flex flex-wrap content-center items-center justify-center gap-x-3 gap-y-1 overflow-hidden rotate-[-8deg] bg-white/70 p-4 backdrop-blur-[1px]">
      ...
      <div className="grid grid-cols-[1fr_1.4fr_auto] items-stretch border-t border-black">
        <div className="flex items-center justify-center border-r border-black p-3">
          <div className="h-10 w-10 rounded-full border border-black transition-transform duration-300 group-hover:scale-110" style={{ background: product.accent }} />
        </div>
        <div className="border-r border-black p-3">
          <p className="text-[10px] uppercase tracking-[0.15em] text-black/60">{product.category}</p>
      ...
      <div className="flex items-start justify-between gap-3 border-t border-black bg-white px-3 py-2">
        ...
        <span className="shrink-0 border border-black bg-black px-3 py-2 text-[11px] font-bold tracking-[0.18em] text-white transition-colors duration-300 group-hover:bg-hot group-hover:text-black">
